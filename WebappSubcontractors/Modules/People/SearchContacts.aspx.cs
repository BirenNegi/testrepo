...
#region Private Methods
        private void BindSearch()
        {
            ...
            {
                contactInfo = (ContactInfo)PeopleController.GetInstance().GetPerson(Int32.Parse(contactInfo.IdStr), null, null);
                SubContractorInfo subcontractorInfo = SubContractorsController.GetInstance().GetSubContractorDeep(int.Parse(contactInfo.SubContractor.IdStr));


                //#-- gvPeople.DataSource = subcontractorInfo.Contacts.FindAll(x=>x.IsInactive==chkInactive.Checked);
                gvPeople.DataSource = subcontractorInfo.Contacts.FindAll(x => x.UserType != "SE");   //#--
                gvPeople.DataBind();


                gvEMployee.DataSource = subcontractorInfo.Contacts.FindAll(x => x.UserType == "SE");   //#--
                gvEMployee.DataBind();
            }
...
#region Event Handlers
        ...
        protected void gvEMployee_OnRowCreated(object sender, GridViewRowEventArgs e)
        {
            Utils.SortedGridSetOrderImage(gvEMployee, e, GvPeopleSortExpression, GvPeopleSortDireccion);
        }
