...
namespace SOS.Web
{
    public partial class ReportPendingTasksIndividualPage : System.Web.UI.Page
    {
...
#region Private Methods
        private void BindReport()
        {
            ...
            EmployeeInfo emp=(EmployeeInfo) Utils.GetCurrentUser();
            ...
            foreach (ProjectInfo project in projectInfoList)
            {
                ...
                selectedUsers.Add(project.Id.Value, empList);
            ...
            foreach (ProjectInfo project in projectInfoList)
            {
                if (pendingProcessSteps.ContainsKey(project.Id.Value))
                {
                    foreach (ProcessStepInfo processStep in pendingProcessSteps[project.Id.Value])
                    {
                        ...
                        if(processStep.AssignedTo!=null)
...
#region Event Handlers
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                Security.CheckAccess(Security.userActions.ViewPendingTasksIndividuals);
