...
namespace SOS.Web
{
    public partial class ReportSubcontractorsVvDv : System.Web.UI.Page
    {
        ...
        private void BindReport()
        {
         //   List<TradeInfo> tradeInfoList = TradesController.GetInstance().SearchTrades(UI.Utils.GetFormString(ddlProjects.SelectedValue), UI.Utils.GetFormString(ddlTrades.SelectedValue), UI.Utils.GetFormInteger(ddlSubbies.SelectedValue));
            ...
            DataTable Dt= projectsController.GetSubcontractorsVvDV(projectId, UI.Utils.GetFormInteger(ddlSubbies.SelectedValue),UI.Utils.GetFormInteger(ddlBusinessUnit.SelectedValue), UI.Utils.GetFormString(ddlTrades.SelectedValue), projectStatus, sdrStartDate.Date,sdrEndDate.Date);




            repSubcontractorsVVDV.LocalReport.ReportPath = Request.PhysicalApplicationPath + "\\Reports\\SubcontractorsVariationsVvDV.rdlc";
            repSubcontractorsVVDV.LocalReport.SetParameters(reportParameterList);
            repSubcontractorsVVDV.LocalReport.DataSources.Clear();
            repSubcontractorsVVDV.LocalReport.DataSources.Add(new ReportDataSource("VvDv", Dt));

            repSubcontractorsVVDV.DataBind();
            repSubcontractorsVVDV.Visible = true;
        }
        #endregion
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!Page.IsPostBack)
                {
                    repSubcontractorsVVDV.Visible = false;
                    BindForm();
                }
            }
            catch (Exception Ex)
